...
const GlobalSearch = async (req, res, next) => {
  try {
    ...
    const [users, videos, series, shorts] = await Promise.all([
      ...
      Series.find({
        $or: [{ title: searchRegex }, { description: searchRegex }, { genre: searchRegex }, { language: searchRegex }],
      })
        .populate("created_by", "username profile_photo")
        .limit(limitNum)
        .skip(skip),
    ...
    const totalResults = users.length + videos.length + series.length + shorts.length;
...
const PersonalizedSearch = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).populate("saved_videos").populate("saved_series").populate("following");
    ...
    const [videos, series, shorts] = await Promise.all([
      ...
      Series.find(seriesQuery).populate("created_by", "username profile_photo").sort({ views: -1, likes: -1 }).limit(limitNum).skip(skip),

      ShortVideos.find({
        $or: [{ name: searchRegex }, { description: searchRegex }, ...(followingIds.length > 0 ? [{ created_by: { $in: followingIds } }] : [])],
      })
        .populate("created_by", "username profile_photo")
        .sort({ views: -1, likes: -1 })
        .limit(limitNum)
...
const GetContentByType = async (req, res, next) => {
  try {
    const { type, limit = 10, page = 1, genre, language, sortBy = "createdAt" } = req.query;

    if (!type) {
      return res.status(400).json({ message: "Content type is required (videos, series, shorts, users)" });
    }

    const validTypes = ["videos", "series", "shorts", "users"];
    if (!validTypes.includes(type)) {
      return res.status(400).json({
        message: "Invalid content type. Valid types are: videos, series, shorts, users",
      });
    }
    ...
    switch (type) {
      ...
      case "series":
        ...
        results = await Series.find(seriesQuery).populate("created_by", "username profile_photo").sort(sortObject).limit(limitNum).skip(skip);

        totalCount = await Series.countDocuments(seriesQuery);
        break;

      case "shorts":
        results = await ShortVideos.find({}).populate("created_by", "username profile_photo").sort(sortObject).limit(limitNum).skip(skip);
