...
const DeleteLongVideo = async (req, res, next) => {
  ...
  try {
    ...
    if (!video.created_by.equals(userId)) {
      return res.status(403).json({ message: "You can only delete videos you created" });
    }

    await Community.updateMany({ long_videos: videoId }, { $pull: { long_videos: videoId } });

    await User.updateMany(
      { $or: [{ saved_videos: videoId }, { playlist: videoId }, { history: videoId }, { liked_videos: videoId }, { video_frame: videoId }] },
      {
        $pull: {
...
const DeleteShortVideo = async (req, res, next) => {
  ...
  try {
    ...
    await Community.updateMany({ short_videos: videoId }, { $pull: { short_videos: videoId } });
...
const DeleteUserProfile = async (req, res, next) => {
  ...
  try {
    ...
    await Community.updateMany({ followers: userId }, { $pull: { followers: userId, creators: userId } });

    await User.updateMany({ $or: [{ followers: userId }, { following: userId }] }, { $pull: { followers: userId, following: userId } });
...
const DeleteCommunity = async (req, res, next) => {
  ...
  try {
    ...
    if (!community.founder.equals(userId)) {
      return res.status(403).json({ message: "Only the founder can delete the community" });
    }

    await User.updateMany(
      { $or: [{ community: communityId }, { my_communities: communityId }, { saved_items: communityId }] },
      {
        $pull: {
...
const DeleteSeries = async (req, res, next) => {
  ...
  try {
    ...
    if (!series.created_by.equals(userId)) {
      return res.status(403).json({ message: "You can only delete series you created" });
    }

    await LongVideo.updateMany({ series: seriesId }, { $unset: { series: 1, episode_number: 1 }, $set: { is_standalone: true, season_number: 1 } });

    await Community.updateMany({ series: seriesId }, { $pull: { series: seriesId } });

    await User.updateMany({ saved_series: seriesId }, { $pull: { saved_series: seriesId } });

    await Series.findByIdAndDelete(seriesId);

    res.status(200).json({ message: "Series deleted successfully. All episodes converted to standalone videos." });
  } catch (error) {
    handleError(error, req, res, next);
  }
};
...
const RemoveVideoFromCommunity = async (req, res, next) => {
  ...
  if (!communityId || !videoId || !videoType) {
    return res.status(400).json({ message: "Community ID, video ID, and video type are required" });
  }
  ...
  try {
    ...
    if (!community.founder.equals(userId) && !community.creators.includes(userId)) {
      return res.status(403).json({ message: "Only founder or creators can remove videos from community" });
    }

    const videoField = videoType === "long" ? "long_videos" : "short_videos";

    if (!community[videoField].includes(videoId)) {
      return res.status(404).json({ message: "Video not found in this community" });
    }

    await Community.findByIdAndUpdate(communityId, { $pull: { [videoField]: videoId } });

    res.status(200).json({ message: `${videoType} video removed from community successfully` });
  } catch (error) {
    handleError(error, req, res, next);
  }
};
...
const UnfollowCommunity = async (req, res, next) => {
  ...
  try {
    ...
    if (!community.followers.includes(userId)) {
      return res.status(400).json({ message: "You are not following this community" });
    }

    await Community.findByIdAndUpdate(communityId, { $pull: { followers: userId } });
...
const RemoveUserFromCommunity = async (req, res, next) => {
  ...
  if (!communityId || !targetUserId) {
    return res.status(400).json({ message: "Community ID and target user ID are required" });
  }
  ...
  try {
    ...
    if (!community.founder.equals(userId)) {
      return res.status(403).json({ message: "Only the founder can remove users from community" });
    }

    if (community.founder.equals(targetUserId)) {
      return res.status(400).json({ message: "Cannot remove the founder from the community" });
    }
...
const BulkDeleteVideos = async (req, res, next) => {
  ...
  try {
    ...
    if (videos.length === 0) {
      return res.status(404).json({ message: "No videos found that you can delete" });
    }

    const validVideoIds = videos.map((video) => video._id);

    if (videoType === "long") {
      await Community.updateMany({ long_videos: { $in: validVideoIds } }, { $pull: { long_videos: { $in: validVideoIds } } });
      ...
      await Community.updateMany({ short_videos: { $in: validVideoIds } }, { $pull: { short_videos: { $in: validVideoIds } } });
    }
