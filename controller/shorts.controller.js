...
const getShortVideoById = async (req, res, next) => {
  try {
    ...
    const shortVideo = await ShortVideo.findById(id).populate("created_by", "username email").populate("community", "name");
...
const updateShortVideo = async (req, res, next) => {
  try {
    ...
    const updatedVideo = await ShortVideo.findByIdAndUpdate(id, updateData, { new: true });
